...
class SignedRequest
  ...
  def self.generate_keypair
    ...
    { signing_key: Base64.strict_encode64(signing_key), verify_key: Base64.strict_encode64(verify_key) }
  end
  ...
  def sign(message)
    ...
    signature = RbNaCl::SigningKey.new(signing_key).sign(message.to_json).then { |sig| Base64.strict_encode64(sig) }
    { data:, signature: }
  end
